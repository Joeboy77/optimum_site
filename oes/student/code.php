...
if (isset($_POST["passwordsubmit"])) {
    ...
    if (empty($oldpassword)) {
        ...
        // Verify old password against the current user's stored password
        $checkQuery = "SELECT password FROM registration WHERE id = '$id' LIMIT 1";
        $checkRun = mysqli_query($con, $checkQuery);
        if ($checkRun && mysqli_num_rows($checkRun) === 1) {
            $userRow = mysqli_fetch_assoc($checkRun);
            if ($userRow['password'] === $oldpassword1) {
                // Update to new password
                $updateQuery = "UPDATE registration SET password='$newpassword1' WHERE id = '$id' LIMIT 1";
